...
@st.cache_data
def get_msl():
    msl = pd.read_parquet("s3://alloc-app-demo/msl.parquet")
    msl["Lookup"] = msl.AllocName + " (" + msl.Ticker + ")"
    return msl

@st.cache_data
def get_returns():
    returns = pd.read_parquet("s3://alloc-app-demo/returns.parquet")
    return returns

@st.cache_data
def get_preset_portfolios():
    return pd.read_parquet("s3://alloc-app-demo/preset_ports.parquet")
